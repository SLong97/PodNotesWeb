        ...
        with st.expander("**Diarized Summary**",expanded=True):
            if expand:    
                with st.spinner("Summarizing Diarized Transcription..."):
                    ...
                    st.session_state.diarize_summary = summary_diarized[0]["Summary"]
                    st.write(st.session_state.diarize_summary)
        ...
        with st.expander("**Save to File/Database**",expanded=True):
            ...
            if download_button:
                if file_type == "PDF":
                    ...
                    if 'diarize_summary' in st.session_state:  
                        create_pdf(st.session_state.diarize_summary, 4)
                    ...
                    if 'diarize_summary' in st.session_state:  
                        create_word_doc(st.session_state.diarize_summary, 4)
                    ...
                    if 'diarize_summary' in st.session_state:  
                        create_text_file(st.session_state.diarize_summary, 4)

                elif file_type == "DATABASE":
                    st.write("**_Successfully Saved To Database_**")
                    save_to_bucket(user_identity, transcript, "Transcript - "+video_title)
                    save_to_bucket(user_identity, summary[0]["Summary"], "Summary - "+video_title)
                    content = "\n".join(saved_diarized_sentences).encode("utf-8")
                    save_to_bucket(user_identity, content, "Diarized Transcript - "+video_title)
                    if 'diarize_summary' in st.session_state:  
                        save_to_bucket(user_identity, st.session_state.diarize_summary, "Diarized Summary - "+video_title)
